...
class _Farm:
    ...
    def __init__(self, n_experts=4, size=2, gate_fn=None, timeout=10.0,
        ...
        for expert in range(n_experts):
            ...
            self.nodes.append(RunningNode(LAYER, expert,
                                          gate_fn(fn) if gate_fn else fn))
        ...
        self.dispatcher = HierarchicalExpertDispatcher(
            self.placement, self.plan, self.transport)

    # -- helpers -----------------------------------------------------------
    def run(self, x, expert_ids, gates, token_id=1, timeout=None):
...
class TestHierarchicalResults(unittest.TestCase):
    ...
    def test_bit_identical_to_in_process_grouped_reduction(self):
        """Deploying real head servers must not change the arithmetic."""
        x = np.linspace(-1.0, 1.0, DIM, dtype=np.float32)
        with _Farm(n_experts=4, size=2) as farm:
            hier = farm.run(x, [0, 1, 2, 3], GATES, token_id=7)
            flat, transport = farm.flat_dispatcher(plan=farm.plan)
            try:
                reference = flat.run_expert_stage(LAYER, x, [0, 1, 2, 3],
                                                  GATES, token_id=8)
            finally:
                flat.close()
                transport.close()
    ...
    def test_single_subcluster_is_bit_identical_to_flat_dispatch(self):
        ...
        with _Farm(n_experts=4, size=4) as farm:
            ...
            flat, transport = farm.flat_dispatcher()
            try:
                reference = flat.run_expert_stage(LAYER, x, [0, 1, 2, 3],
                                                  GATES, token_id=12)
            finally:
    ...
    def test_bad_flags_get_a_bad_request_error_and_the_connection_survives(self):
        ...
        with _Farm(n_experts=2, size=2) as farm:
            ...
            try:
                ...
                np.testing.assert_allclose(good["array"],
                                           farm.weights[0] @ x,
                                           rtol=1e-5, atol=1e-5)
            finally:
                sock.close()
